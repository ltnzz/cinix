...
export default function AdminDashboardPage() {
  ...
  const [refreshKey, setRefreshKey] = useState(0); // Trigger untuk refresh data otomatis
  const [searchTerm, setSearchTerm] = useState(""); // Fitur pencarian sederhana
  ...
  // 1. Hack untuk menyembunyikan Footer website utama saat di halaman admin
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchMovies = async () => {
      ...
      try {
        ...
        // Handle struktur response yg mungkin berbeda (res.data atau res.data.data)
  ...
  const handleLogout = () => {
    ...
    // Call API Logout (opsional, tergantung backend)
  ...
  // Filter film berdasarkan search
    ...
    <div className="min-h-screen bg-[#f5f1dc] font-sans flex">
      ...
      <aside className="fixed top-0 left-0 h-screen w-64 bg-[#2a4c44] text-white flex flex-col z-30 shadow-2xl">
        ...
        <nav className="flex-1 p-4 overflow-y-auto mt-4">
          ...
          {/* Anda bisa menambah menu lain di sini */}
        </nav>
            ...
            {filteredMovies.map((movie, idx) => (
                <div key={idx} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-[#2a4c44]/10 transition-all duration-300 group border border-gray-100 flex flex-col h-full">
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                        ...
                        {/* Mobile Actions (Visible only on small screens or fallback) */}
